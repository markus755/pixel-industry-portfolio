...
class PreGeneratedAudioPlayer {
    ...
    /**
     * Berechnet Audio-Dateinamen mit Hash
     */
    async calculateAudioFileName() {
        // MD5 Hash des Textes (gleiche Logik wie Build-Script)
        const msgBuffer = new TextEncoder().encode(this.textContent);
        const hashBuffer = await crypto.subtle.digest('SHA-256', msgBuffer);
        const hashArray = Array.from(new Uint8Array(hashBuffer));
        const hashHex = hashArray.map(b => b.toString(16).padStart(2, '0')).join('');
        const hash = hashHex.substring(0, 8);
